...
local asset_mgr = {
    get = function(x)
        local y = "rbxasset://NebulaXExploit/"
        if not iscustomasset(x) then
       
            
            local URL = "https://raw.githubusercontent.com/DancingUnicornLol/NebulaXExec/refs/heads/main/assets/" .. x .. ".png";
...
local iconroni_id = "rbxasset://NebulaXExploit/nebulax_icon123.png";
if not iscustomasset("nebulax_icon123.png") then
    local data = game:HttpGet("https://raw.githubusercontent.com/DancingUnicornLol/NebulaXExec/refs/heads/main/Untitled_Artwork.png");
    assert(writecustomasset("nebulax_icon123.png", data) == iconroni_id, "icons got messed up, report this");
end
...
UI["79"]["Text"] = [[🔴 <font color="#B62a3d">14:09:20 
...
local script = UI["3b"]
	...
	local nebulaxButton = script.Parent.Parent.Parent:FindFirstChild("NebulaXButton")
	local lastPosition = nil;
	
	if closeButton and nebulaxButton then
		...
		local function moveButton(targetPosition)
			local tween = TweenService:Create(nebulaxButton, tweenInfo, {Position = targetPosition})
			tween:Play()
		end
		...
		closeButton.MouseButton1Click:Connect(function()
			...
			if lastPosition ~= nil then
                nebulaxButton.Position = lastPosition;
            end
		end)
	
		nebulaxButton.MouseButton1Click:Connect(function()
			moveButton(upPosition)
			
			lastPosition = nebulaxButton.Position;
		end)
	end
end
...
local script = UI["d7"]
	...
	local nebulaxButton = script.Parent:WaitForChild("NebulaXButton")
	configButton.MouseButton1Click:Connect(function()
		nebulaxButton.Visible = true;
		Hidden()
		Close()
	end)
	
	nebulaxButton.MouseButton1Click:Connect(function()
		nebulaxButton.Visible = false;
		Shown()
	end)
